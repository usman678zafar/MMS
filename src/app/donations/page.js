...
import { addDonation, getDonations, getDonors } from './actions';
...
export default function DonationsPage() {
  ...
  const handleAdd = async (e) => {
    ...
    const res = await addDonation({ ...newDonation, receipt_url: url });
    if (res.success) { setShowModal(false); setNewDonation(defaultForm); setSelectedFile(null); fetchDonations(); }
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowModal(true)} className="btn btn-primary text-sm"><Plus className="h-4 w-4 mr-1.5" />Add Donation</button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-50">
            <div className="relative max-w-sm">
              ...
              <input type="text" placeholder="Search donor..." className="w-full pl-10 pr-4 py-2 bg-slate-50 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  {['Date','Donor','Type','Amount','Receipt',''].map(h => (
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? <tr><td colSpan="6" className="px-5 py-10 text-center text-slate-400">Loading...</td></tr>
                  : donations.length === 0 ? <tr><td colSpan="6" className="px-5 py-10 text-center text-slate-400">No donations found.</td></tr>
                  : donations.map((d) => (
        ...
        <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Donation">
          <form onSubmit={handleAdd} className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Receipt Image</label>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button type="submit" disabled={uploading} className="btn btn-primary text-sm disabled:opacity-50">{uploading ? 'Uploading...' : 'Save Donation'}</button>
            </div>
          </form>
        </Modal>
      </div>
