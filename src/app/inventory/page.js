...
import { addInventoryItem, getInventory } from './actions';
...
export default function InventoryPage() {
  ...
  const handleAddItem = async (e) => {
    ...
    if (res.success) { setShowModal(false); fetchInventory(); setNewItem({ item_name: '', category: 'General', quantity: '', unit: 'pcs' }); }
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-slate-50">
            <div className="relative max-w-sm">
              ...
              <input type="text" placeholder="Search items..." className="w-full pl-10 pr-4 py-2 bg-slate-50 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  {['Item Name','Category','Stock Level','Status',''].map(h => (
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? <tr><td colSpan="5" className="px-5 py-10 text-center text-slate-400">Loading...</td></tr>
                  : items.length === 0 ? <tr><td colSpan="5" className="px-5 py-10 text-center text-slate-400">No items found.</td></tr>
                  : items.map((item) => (
        ...
        <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Inventory Item">
          <form onSubmit={handleAddItem} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="input-field text-sm" value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})}>
                  {['General', 'Cleaning', 'Educational', 'Office', 'Kitchen'].map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button type="submit" disabled={saving} className="btn btn-primary text-sm disabled:opacity-50">{saving ? 'Saving...' : 'Add Item'}</button>
            </div>
          </form>
        </Modal>
      </div>
