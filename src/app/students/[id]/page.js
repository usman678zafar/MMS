              ...
              <div className="space-y-6">
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
                  ...
                  <div className="mt-5 overflow-x-auto">
                    {data.attendance.length === 0 ? <EmptyState icon={CalendarCheck} title="No attendance records" description="Attendance history will appear here." /> : <table className="w-full min-w-[560px] text-left"><thead><tr className="border-b border-slate-100 text-[10px] uppercase tracking-wide text-slate-400"><th className="px-3 py-3">Date</th><th className="px-3 py-3">Status</th><th className="px-3 py-3">Notes</th></tr></thead><tbody className="divide-y divide-slate-50">{data.attendance.map((entry) => <tr key={entry.id}><td className="px-3 py-4 text-sm font-semibold text-slate-700">{formatDate(entry.date, "EEEE, MMM d, yyyy")}</td><td className="px-3 py-4"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${attendanceTone[entry.status] || "bg-slate-50 text-slate-600"}`}>{entry.status}</span></td><td className="px-3 py-4 text-sm text-slate-500">{entry.notes || "—"}</td></tr>)}</tbody></table>}
                  </div>
                </section>
              </div>
            )}
            ...
            {activeTab === "fees" && (
              <div className="space-y-6">
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
                  ...
                  <div className="mt-5 overflow-x-auto">
                    {data.fees.length === 0 ? <EmptyState icon={Receipt} title="No payments recorded" description="Fee payments and receipt details will appear here." /> : <table className="w-full min-w-[620px] text-left"><thead><tr className="border-b border-slate-100 text-[10px] uppercase tracking-wide text-slate-400"><th className="px-3 py-3">Period</th><th className="px-3 py-3">Paid on</th><th className="px-3 py-3">Notes</th><th className="px-3 py-3 text-right">Amount</th></tr></thead><tbody className="divide-y divide-slate-50">{data.fees.map((entry) => <tr key={entry.id}><td className="px-3 py-4 text-sm font-bold text-slate-800">{entry.month} {entry.year}</td><td className="px-3 py-4 text-sm text-slate-500">{formatDate(entry.date)}</td><td className="px-3 py-4 text-sm text-slate-500">{entry.notes || "—"}</td><td className="px-3 py-4 text-right text-sm font-extrabold text-emerald-600">{currency(entry.amount)}</td></tr>)}</tbody></table>}
                  </div>
                </section>
              </div>
            )}
